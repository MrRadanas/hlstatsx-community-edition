...
if ($player_id > 0) {
    ...
    if (($playerdata['activity'] > 0) && (0 == $playerdata['hideranking'])) {
        $rank = get_player_rank($playerdata);
    } else {
        if (1 == $playerdata['hideranking']) {
            $rank = 'Hidden';
        } elseif (2 == $playerdata['hideranking']) {
            $rank = 'Banned';
        } else {
            $rank = 'Not active';
        }
    ...
    $background = 'random';
    if (isset($_GET['background']) && ((($_GET['background'] > 0) && ($_GET['background'] < 12)) || ('random' == $_GET['background']))) {
    ...
    if (0 == $playerdata['last_skill_change']) {
    ...
    imagestring($image, 2, 85, 56, $g_options['siteurl'], $link_color);
