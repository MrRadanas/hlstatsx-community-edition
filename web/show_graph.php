...
*/
	...
	$bg_color = ['red' => 171, 'green' => 204, 'blue' => 214];
	...
	if ($bar_type == 0)
	{
		...
		{
			$i++;
			$avg_values[] = ['timestamp' => $rowdata['timestamp'], 'act_players' => $rowdata['act_players'], 'min_players' => $rowdata['min_players'], 'max_players' => $rowdata['max_players'], 'uptime' => $rowdata['uptime'], 'fps' => $rowdata['fps'], 'map' => $rowdata['map']];
			...
			if ($i == $avg_step)
			{
				...
				$data_array[] = ['timestamp' => $insert_values['timestamp'], 'act_players' => $insert_values['act_players'], 'min_players' => $insert_values['min_players'], 'max_players' => $insert_values['max_players'], 'uptime' => $insert_values['uptime'], 'fps' => $insert_values['fps'], 'map' => $insert_values['map']];
				$avg_values = [];
				$i = 0;
			}
		...
		if ($avg_step == 1)
		{
			...
			$rowdata['uptime'] = 0;
			array_unshift($data_array, ['timestamp' => time(), 'act_players' => $rowdata['act_players'], 'min_players' => $data_array[0]['min_players'], 'max_players' => $rowdata['max_players'], 'uptime' => $rowdata['uptime'], 'fps' => $rowdata['uptime'], 'map' => $last_map]);
		}

		if (count($data_array) > 1)
		{
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'max_players', 0, 1, 0, 1, [$gray, $red, $font_color, $dark_color, $light_gray]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'min_players', 0, 0, 0, 1, [$dark_color, $red, $font_color, $dark_color]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'act_players', 0, 0, 1, 1, [$blue, $red, $font_color, $dark_color]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 2, 'uptime', 0, 0, 1, 1, [$orange, $red, $font_color, $dark_color]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 2, 'fps', 0, 0, 1, 1, [$red, $red, $font_color, $dark_color]);

			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'max_players', 0, 1, 0, 0, [$gray, $red, $font_color, $dark_color, $light_gray]);
		}
	...
	} elseif ($bar_type == 1)
	{
		...
		while ($rowdata = $db->fetch_array($result))
		{
			$data_array[] = ['timestamp' => $rowdata['timestamp'], 'players' => $rowdata['players'], 'kills' => $rowdata['kills'], 'headshots' => $rowdata['headshots'], 'act_slots' => $rowdata['act_slots'], 'max_slots' => $rowdata['max_slots']];
		}
		...
		if (count($data_array) > 1)
		{
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'kills', 0, 0, 0, 0, [$orange, $red, $font_color, $dark_color, $light_gray]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'headshots', 0, 0, 0, 1, [$dark_color, $red, $font_color, $dark_color, $light_gray]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'players', 0, 0, 0, 1, [$red, $red, $font_color, $dark_color, $light_gray]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 2, 'max_slots', 0, 0, 0, 1, [$gray, $red, $font_color, $dark_color, $light_gray]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 2, 'act_slots', 0, 0, 1, 1, [$blue, $red, $font_color, $dark_color]);

			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'kills', 0, 1, 0, 1, [$orange, $red, $font_color, $dark_color, $light_gray]);
		}
	...
	} elseif ($bar_type == 2)
	{
		...
		while (($rowdata = $db->fetch_array($result)) && ($arcount < $deletedays))
		{
			...
			{
				// insert null value
				$data_array[] = ['timestamp' => $ts, 'skill' => $rowdata['skill'], 'kills' => 0, 'headshots' => 0, 'deaths' => 0, 'time' => 0];
				$ts -= 86400;
				$arcount++;
			}

			$data_array[] = ['timestamp' => $rowdata['ts'], 'skill' => $rowdata['skill'], 'kills' => $rowdata['kills'], 'headshots' => $rowdata['headshots'], 'deaths' => $rowdata['deaths'], 'time' => $rowdata['connection_time']];
			$arcount++;
			$ts -= 86400;
		}
		...
		if (count($data_array) > 1)
		{
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'kills', 0, 1, 0, 1, [$red, $red, $font_color, $dark_color, $light_gray]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'headshots', 0, 0, 0, 1, [$dark_color, $red, $font_color, $dark_color, $light_gray]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 0, 'skill', 0, 0, 0, 1, [$orange, $red, $font_color, $dark_color, $light_gray]);
			drawItems($image, ['width' => $width, 'height' => $height, 'indent_x' => $indent_x, 'indent_y' => $indent_y], $data_array, 2, 'deaths', 0, 0, 0, 1, [$gray, $red, $font_color, $dark_color, $light_gray]);
		}
