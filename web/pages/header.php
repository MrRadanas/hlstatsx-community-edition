...
// visit counter
if (isset($_COOKIE['ELstatsNEO_Visit']) && 0 == $_COOKIE['ELstatsNEO_Visit']) {
    ...
    @setcookie('ELstatsNEO_Visit', '1', ['expires' => time() + ($g_options['counter_visit_timeout'] * 60), 'path' => '/']);
}
...
if (isset($_POST['stylesheet']) || isset($_COOKIE['style'])) {
    setcookie('style', $selectedStyle, ['expires' => time() + 60 * 60 * 24 * 30]);
}
...
<?php
    ...
    if (isset($_POST['js']) && $_POST['js']) {
        ...
        if ((!isset($_SESSION['nojs'])) or (1 == $_SESSION['nojs'])) {
...
if ($num_games > 1 && 1 == $g_options['display_gamelist']) {
    ?>
...
<?php
        if (1 == $g_options['display_style_selector']) {
            ?>
...
<?php
    global $mode;
if (0 != $g_options['bannerdisplay'] && ('contents' == $mode || 1 == $g_options['bannerdisplay'])) {
    ?>    
	<div class="block" style="text-align:center;">
		<img src="<?php echo (0 == strncmp($g_options['bannerfile'], 'http:/', 6)) ? $g_options['bannerfile'] : IMAGE_PATH.'/'.$g_options['bannerfile']; ?>" alt="Banner" />
	</div>
<?php
}
?>        
