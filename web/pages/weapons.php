...
$result                      = $db->query('
		SELECT
			...
			ROUND(hlstats_Weapons.kills / '.((0 == $realkills) ? 1 : $realkills).' * 100, 2) AS kpercent,
			hlstats_Weapons.headshots,
			ROUND(hlstats_Weapons.headshots / IF(hlstats_Weapons.kills = 0, 1, hlstats_Weapons.kills), 2) AS hpk,
			ROUND(hlstats_Weapons.headshots / '.((0 == $realheadshots) ? 1 : $realheadshots)." * 100, 2) AS hpercent,
