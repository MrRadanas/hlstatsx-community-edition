...
function hide_cheaters($query)
{
    ...
    $insert_part = '';
    $first       = 0;
    while ([$player_id] = $db->fetch_row($result)) {
        if (0 == $first) {
...
if (isset($_POST['confirm'])) {
    ...
    while (socket_select($read, $write = null, $except = null, $timeout) > 0) {
        ...
        if (($buffer[0] == chr(255)) && ($buffer[1] == chr(255)) && ('Z' == $buffer[2]) && ($buffer[3] == chr(255))
            && ('1' == $buffer[4]) && ('.' == $buffer[5]) && ('0' == $buffer[6]) && ('0' == $buffer[7]) && ($buffer[8] == chr(255))) {
